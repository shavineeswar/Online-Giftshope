...
class AddSupplierItems extends Component {
    ...
    componentDidMount() {
        axios.get(`http://localhost:9999/supplieritems/item/${this.props.match.params.id}`)
            .then(response => {
                ...
                this.setState({ imgURL: response.data.data.imgURL })
    ...
    onSubmit(e) {
        ...
        storageRef.child('images/' + this.state.files[0].name).getDownloadURL().then((url) => {
            ...
            axios.put(`http://localhost:9999/supplieritems/edit/${this.props.match.params.id}`, giftItem)
                .then(response => {
